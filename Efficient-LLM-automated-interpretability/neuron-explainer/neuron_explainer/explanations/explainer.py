...
from neuron_explainer.activations.activation_records import (
    ...
    highlight_with_summary,
    AVHS_activation_records
)
...
HARMONY_V4_MODELS = ["gpt-3.5-turbo", "gpt-4", "Meta-Llama-3.1-8B-Instruct"]
...
class NeuronExplainer(ABC):
    ...
    async def generate_explanations(
        ...
        **prompt_kwargs: Any,
        ...
        #
        if to_print:
            print("Printing explanation prompt")
            print("-")
...
class SummaryExplainer(NeuronExplainer):
    ...
    def make_explanation_prompt(self, **kwargs: Any) -> Union[str, list[HarmonyMessage]]:
        ...
        prompt_builder.add_message(
            ...
            "Look at the parts/tokens of the document the neuron activates highly for "
            "and summarize in a single sentence what the neuron is looking for. " 
            "Don't list examples of words.\n\n"
            "We will show short text excerpts, followed by a comma separated list of tokens(part of word) "
            "that activate highly in those text excerpts. Your task is to summarize what the highly activating "
            "tokens have in common, taking their context into account.",
        )
        few_shot_examples = self.few_shot_example_set.get_examples()
        num_omitted_activation_records = 0
        for i, few_shot_example in enumerate(few_shot_examples):
...
class HighlightExplainer(NeuronExplainer):
    ...
    def make_explanation_prompt(self, **kwargs: Any) -> Union[str, list[HarmonyMessage]]:
        ...
        prompt_builder.add_message(
            ...
            "and summarize in a single sentence what the neuron is looking for. " 
            "Don't list examples of words.\n\n"
            "We will show short text excerpts, where each highly activating token(part of word) is highlighted "
            "using square brackets, i.e. [token]. Your task is to summarize what the highly activating "
            "tokens have in common, taking their context into account.",
        )
        few_shot_examples = self.few_shot_example_set.get_examples()
        num_omitted_activation_records = 0
...
class HighlightSummaryExplainer(NeuronExplainer):
    """
    Generate explanations of neuron behavior using a prompt with lists of token/activation pairs.
    """
    ...
    def make_explanation_prompt(self, **kwargs: Any) -> Union[str, list[HarmonyMessage]]:
        ...
        prompt_builder.add_message(
            ...
            "Don't list examples of words.\n\n"
            "We will show short text excerpts, where each highly activating token(part of word) is highlighted "
            "using square brackets, i.e. [token]. This is followed by a comma separated list of only the highly activating tokens. "
            "Your task is to summarize what the highly activating "
            "tokens have in common, taking their context into account.",
        )
        few_shot_examples = self.few_shot_example_set.get_examples()
        num_omitted_activation_records = 0
...
Neuron {index + 1}
Activations:{highlight_with_summary(activation_records, cutoff)}"""
        # We repeat the non-zero activations only if it was requested and if the proportion of
        # non-zero activations isn't too high.
...
class AVHSExplainer(NeuronExplainer):
    """
    Generate explanations of neuron behavior using a prompt with lists of token/activation pairs.
    """
    ...
    def make_explanation_prompt(self, **kwargs: Any) -> Union[str, list[HarmonyMessage]]:
        ...
        prompt_builder.add_message(
            ...
            "Don't list examples of words.\n\n"
            "We will show short text excerpts, where each highly activating token(part of word) is highlighted "
            "using square brackets, i.e. [token], followed by a comma separated list of tokens(part of word) and activation value pairs "
            "that activate highly in those text excerpts. "
            "The activation format is token<tab>activation. "
            "Activation values range from 0 to 10. A neuron finding what it's looking for is represented by a "
            "non-zero activation value. The higher the activation value, the stronger the match. "
            "Your task is to summarize what the highly activating "
            "tokens have in common, taking their context into account.",
        )
        ...
        for i, few_shot_example in enumerate(few_shot_examples):
            ...
            self._add_per_neuron_explanation_prompt(
                ...
                few_shot_activation_records,
                i,
                max_activation = calculate_max_activation(few_shot_example.activation_records),
                numbered_list_of_n_explanations=numbered_list_of_n_explanations,
                explanation=few_shot_example.explanation,
            )
        self._add_per_neuron_explanation_prompt(
...
Neuron {index + 1}
Activations:{AVHS_activation_records(activation_records, max_activation)}"""
        # We repeat the non-zero activations only if it was requested and if the proportion of
        # non-zero activations isn't too high.
        if (
            self.repeat_non_zero_activations
